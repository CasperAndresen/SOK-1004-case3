...
df_vekt %>%
  group_by(dato) %>%
  summarize(sum_av_vekt = sum(value)) %>%
  mutate(avvik = 1 - sum_av_vekt)
```
...
df_wide <- rename(df_wide, indeks = 3)
df_wide <- rename(df_wide, vekter = 4)

df_wide$total <- as.Date(df_wide$dato)
aggregate(df_wide["indeks"], by=df_wide["total"], sum)

#df_wide <- df_wide %>%
#  group_by(dato)

#df_wide <- df_wide %>% summarise(across(c("indeks", "vekter")))

#df_wide <- df_wide %>% mutate(total = indeks + vekter)
...
df %>%
  ...
  mutate(prosent = 100*(value - lag(value))/lag(value)) %>%
...
*Grafen viser vekstbidraget til konsumgruppen elektrisitet, inkludert nettleie i prosent over ca. 11 år. Variasjonen er tatt i forhold til prisen i 2015.*
...
*Høyere kraftpriser vil føre til høyere kostnader når det kommer til produksjon av diverse produkter. Dette vil igjen føre til at produsenter øker prisene sine for å kompensere tapet deres.*
